...
+ 自动排序：只按考试的**开始时间**进行升序 (早到晚) 排列。不允许用户自定义顺序。
...
2. 程序内部出错，请运行 `eventvwr.msc` 定位到 `Windows 日志` >`应用程序` 找到当时来源为 `.NET Runtime` 或 `Application Error` 的**错误**消息，确保来自，点击下方的**详细信息**选项卡，复制所有日志内容，然后上报到 [Issues](https://github.com/WangHaonie/PlainCEETimer/issues/new/choose)。
## 无法设置/取消开机启动
