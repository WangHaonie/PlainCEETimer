...
> + 版本号沿用上一个正式版的版本号，确保自动构建版在下一个正式版发布时可以自动更新
...
+ 重复判断依据：`考试名称`和`开始日期时间`相等。
+ 内容超出列表视图范围时将鼠标悬停倒项目上将以工具提示 (ToolTip) 展示对应内容。
...
| **选项名称** | **功能说明** | **注意** |
| ----- | ---- | ---- |
| **列表 (`ListView`)** | 展示考试信息。 | 右键单击表格打开快捷菜单进行相关操作。 |
...
## 🎉 特性
+ 自动排序功能：先按规则**类别**排序，再按**时刻**降序排列 (近到远)。若规则**类别**为第三阶段，则按**时刻**升序 (远到近) 排列。不允许用户自定义顺序。
+ 重复判断依据：`类别`和`时刻`相等。
...
| **选项名称** | **功能说明** | **注意** |
| ----- | ---- | ---- |
| **列表 (`ListView`)** | 展示自定义规则。 | 右键单击表格打开快捷菜单进行相关操作。 |
| **操作** | 在当前位置显示表格的快捷菜单。 | 适用于无法右键单击表格打开菜单的特殊情况，而左键单击此按钮就可以显示菜单。 |
...
## 双击打开后无界面显示且疑似闪退/其他异常致卡死无法弹出错误消息框等
