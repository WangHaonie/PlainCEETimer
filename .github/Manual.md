<div align="center">
    ...
    <h4>适用于 v5.0.1 | 修订日期 2025/5/10</h4>
</div>
...
# 🎯 托盘图标使用说明
## 🎉 特性
+ 当鼠标悬停在图标上时，可显示当前倒计时的内容。(默认关闭，需手动开启。在 Windows 7 上无法实时更新属于**正常现象**。)
...
| **重启倒计时** | 主要用于更改了屏幕缩放之后, 可以点击此按钮来重启程序以确保 UI 正常显示。 | 由于无法支持 `Per-Monitor V2`，需手动重启才能避免模糊。 |
| **应用主题设置** | 用于更改除倒计时窗口以外的 UI 界面主题。可选 跟随系统/浅色/深色。 | 深色主题目前覆盖率 95%。仅支持 Windows 10 1903 及以上并且关闭了系统高对比度主题。跟随系统并不支持在运行时随系统主题的切换而切换，需手动重启的倒计时。 |
...
## 🎉 特性
+ 自动排序功能：只按考试的**开始时间**进行升序 (早到晚) 排列。不允许用户自定义顺序。
+ 重复项判断依据：两个考试信息的`考试名称`和`开始日期时间`相等。
...
## 🎉 特性
+ 自动排序功能：先按规则**类别**分组，再按**时刻**降序排列。若规则**类别**为第三阶段，则按**时刻**升序排列。不允许用户自定义顺序。
...
程序内部出错，请运行 `eventvwr.msc` 定位到 `Windows 日志` >`应用程序` 找到当时来源为 `.NET Runtime` 的**错误**消息，点击下方的**详细信息**选项卡，复制所有日志内容，然后上报到 [Issues](https://github.com/WangHaonie/PlainCEETimer/issues/new/choose)。
## 提权失败 当前系统已禁用 UAC
